...
class String
  def roman_to_hiragana
    Kconv.kconv(self.toutf8.split(/([a-zA-Z]+)/u).map{|e|e =~ /[a-zA-Z]+/u?NKF.nkf("-wh1",RomanKana.romankana(e)):e}.join(''),Kconv.guess(self),Kconv::UTF8)
  end
  def roman_to_katakana
    Kconv.kconv(self.toutf8.split(/([a-zA-Z]+)/u).map{|e|e =~ /[a-zA-Z]+/u?RomanKana.romankana(e):e}.join(''),Kconv.guess(self),Kconv::UTF8)
  end
  ...
  def to_hiragana
    Kconv.kconv(NKF.nkf("-wh1",RomanKana.romankana(self)),Kconv.guess(self),Kconv::UTF8)
  end
  def to_katakana
    Kconv.kconv(NKF.nkf("-wh2",RomanKana.romankana(self)),Kconv.guess(self),Kconv::UTF8)
  end
end
